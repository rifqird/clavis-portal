...
function AccountMoveCard() {
    ...
    useEffect(() => {
        axios
            .get(`${__API_URL__}/account_moves/get_revenue_vs_expense`)
    ...
    useEffect(() => {
        ...
        const seriesData1 = accountMoves.map(item => Number(item.revenue_total));
        const seriesData2 = accountMoves.map(item => Number(item.expense_total));
        ...
        const options = {
            ...
            series: [
                ...
                { name: "Expenses", data: seriesData2 },
            ],
            xaxis: {
    ...
    return (
        <div ref={chartRef} id="apex_area12"></div>
    );
}
